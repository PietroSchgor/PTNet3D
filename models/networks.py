...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                real_tensor = torch.full(input.size(), self.real_label, dtype=torch.float32, device=input.device)
            ...
            if create_label:
                fake_tensor = torch.full(input.size(), self.fake_label, dtype=torch.float32, device=input.device)
